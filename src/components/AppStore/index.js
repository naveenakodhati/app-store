...
class AppStore extends Component {
  state = {inputValue: '', activeTab: tabsList[0].tabId}
  ...
  onChangeTabs = () => {
    ...
    const filteredTab = appsList.filter(
      eachAppItem =>
        eachAppItem.category === activeTab &&
        eachAppItem.appName.toLowerCase().includes(inputValue),
    )
    return filteredTab
  }
  ...
  render() {
    ...
    const filteredTabData = this.onChangeTabs()
    ...
    return (
      <div className="bg-container">
        ...
        <ul className="app-container">
          {filteredTabData.map(eachList => (
            <AppItem tabDataDetails={eachList} key={eachList.appId} />
          ))}
        </ul>
      </div>
    )
